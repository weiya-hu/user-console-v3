      ...
      >
        <div class="flexl permit_page_operate">
          <el-checkbox
            ...
            :key="itm.permit_id"
            class="permit_page_operate_check"
            :label="itm.permit_id"
            >{{ itm.permit_name }}</el-checkbox
          >
        </div>
      </el-checkbox-group>
    </div>
...
watch(checkAll, (newName) => {
  ...
  for (const j in isIndeterminate.value) {
...
const getIsChecked = (arr: any, obj: any) => {
  ...
  arr.forEach((item: any) => {
    ...
    object[item.checkAll.permit_id] =
      obj[item.checkAll.permit_id].length > 0 &&
      obj[item.checkAll.permit_id].length < item.children.length
  })
  return [objt, object]
}
  ...
  props.item.forEach((item) => {
    ...
    item.pid &&
      (() => {
        ...
        obj[item.pid] = []
        checks[item.pid] = checks[item.pid]
          ? checks[item.pid].concat(item.permit_id)
          : [item.permit_id]
      })()
  })
...
const CheckedPermitChange = (value: any, permitId: any) => {
  ...
  checkAll.value = {
    ...checkAll.value,
    [permitId]: checkedCount === checkes.value[permitId].length,
  }
  isIndeterminate.value = {
    ...isIndeterminate.value,
    [permitId]: checkedCount > 0 && checkedCount < checkes.value[permitId].length,
  }
