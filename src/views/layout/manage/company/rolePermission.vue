<template>
  <div class="flexb role_permission">
    ...
    <KzDialog v-model="changePersonRoleShow" title="变更角色" @sure="personChangedRole">
      <div class="flexl person_change_role">
        <div>角色</div>
        <el-select
          v-model="personChangedRoles"
          class="person_change_role_select"
          multiple
    ...
    <el-dialog
      ...
      title="添加人员"
      custom-class="add_person_dialogo_hwy"
      @close="addPersonList = []"
    >
      <div class="add_person flex">
        <div class="person_tree">
          <div class="person_tree_menu flexl">
            <div
              ...
              :key="i"
              :class="
                addPersonMenuActive === i
                  ? 'add_person_menu_active add_person_menu_item'
            ...
            <div
              class="person_tree_menu_line"
              :style="'width:' + menu_line_width + 'px;' + 'left:' + menu_line_left + 'px'"
            ></div>
          </div>
          <div class="person_list">
            <div v-if="!addPersonMenuActive">
              ...
              <el-scrollbar class="person_list">
                <el-empty
                  v-if="JSON.stringify(currentRolePersonList) === '{}'"
                  description="暂无数据"
                />
                <el-checkbox-group v-else v-model="addPersonList">
                  <div>
                    <el-checkbox
        ...
        <div class="person_selected">
          ...
          <div class="flexl person_checked_list">
            <div v-for="item in addPersonList" :key="item" class="person_checked flexb">
              {{ allPerson[item] }}
...
import {
  ...
  permitRoleList_api,
  companyRoleNameUp_api,
  permitRoleAdd_api,
  companyRoleDel_api,
  companyRoleIn_api,
  companyMemberList_api,
  companyMemberDo_api,
  companyMemberNameGet_api,
  companyMemberRoleListUp_api,
  companyUserTwoPage_api,
  companyRoleIdsGet_api,
  companyRoleIdDel_api,
  companyRoleIdsUp_api,
} from '@/api/manage/company/rolePermit'
...
const addPersonList = ref<any[]>([]) //添加列表
...
const currentPersonId = ref() //当前人眼的id
const objH: any = {}
...
const getRoleList = async () => {
  ...
  status &&
    (() => {
      roleAll.value = body
      const urlRoleId = getUrlParam('roleId')
      const roleIndexs = Object.keys(body)
...
const permitCheckedListed = computed(() => {
  const arr: any = {}
  permitCheckedList.value.length > 0 &&
    permitCheckedList.value.forEach((item: any) => {
      arr[item.instance] = {
        ...arr[item.instance],
        [item.pid]:
          arr[item.instance] && arr[item.instance][item.pid]
            ? arr[item.instance][item.pid].concat(item.permit_id)
            : [item.permit_id],
      }
    })
...
//当前人员确认变更角色按钮
const personChangedRole = async () => {
  const data = {
    role_ids: personChangedRoles.value,
    member_id: currentPersonId.value,
  }
  console.log(data)
  const { status } = await companyRoleIdsUp_api(data)
  status &&
    (() => {
      changePersonRoleShow.value = false
      roleTogglePerson()
    })()
}
...
const roleDeletePerson = (id: any) => {
  ElMessageBox.confirm('是否确认从当前角色中删除此人员', '操作提示', {
    ...
    const { status } = await companyRoleIdDel_api(data)
    status &&
      (() => {
        roleTogglePerson()
      })()
...
const addRoleNameDo = async () => {
  const { status } = await companyRoleIn_api({ role_name: addRoleName.value })
...
const deleteCheckedPerson = (item: any) => {
  const arr = JSON.parse(JSON.stringify(addPersonList.value))
  const index = arr.indexOf(item)
  if (checkTreeRef.value) {
    const checkedChildren = getChildrenKeys(checkTreeRef.value.getCheckedKeys())
