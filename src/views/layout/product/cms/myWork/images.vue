<template>
  <div class="m_images">
    ...
    <el-dialog v-model="addShow" title="上传图片" width="400px" @close="close" custom-class="upimgs">
...
import { imagesList_api, imagesAdd_api, imagesDel_api, imagesEdit_api } from '@/api/product/cms/myWork'
...
const upOneImg = async (file:UploadFile)=>{
  ...
  if(res.status == 1){
    ...
    if(response.status == 200){
      ...
      const oneRes:IRes = await imagesAdd_api({ thumb_url: oneUrl, source_name: fxname.slice(0,30) })
      if(oneRes.status == 1){
