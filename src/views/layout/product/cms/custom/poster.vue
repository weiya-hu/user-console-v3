<template>
  <div class="kz_card">
              ...
              <template #default="{ row }">
                <div class="link_bts">
                  <el-link
                    v-if="row.status == 4"
                    type="primary"
                    @click="$router.push('/product/cms/custom/posterdetail?id=' + row.id)"
                    >详情</el-link
                  >
                  <el-link
                    v-if="row.status == 3"
    ...
    <KzUpUser v-model="dialogVisible" :type="3" @submitSuccess="submitsuccess" />
...
const sureDel = () => {
  //确认删除
  customDel_api({ id: delId.value }, 3).then((res) => {
    if (res.status === 1) {
      getList()
      delShow.value = false
    }
