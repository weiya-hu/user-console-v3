...
import { customDetails_api, customPosterDel_api } from '@/api/product/cms/custom'
...
const tableTitle = ref([
  ...
  { type: 'img', lable: '海报', prop: 'thumb_url', width: 150 },
...
const delImags = async () => {
  const { status } = await customPosterDel_api({ id: delId.value })
  if (status === 1) {
    getList()
    delShow.value = false
  }
