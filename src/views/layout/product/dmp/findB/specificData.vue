  ...
  <div class="dmp_page kz_card">
      ...
      <el-table :data="tableData" style="width: 100%" @selection-change="handleSelectionChange">
        ...
        <el-table-column property="group_desc" label="人群描述" width="300">
          <template #default="{ row }">
            <el-tooltip effect="dark" placement="top">
              <template #content>
                <div style="width: 100px" class="fcc">{{ row.group_desc }}</div>
              </template>
              <div class="els2">{{ row.group_desc }}</div>
            </el-tooltip>
          </template>
        </el-table-column>
          ...
          <template #default="scope">
            <div v-if="scope.row.status == 1" class="fcs">
              <el-link v-if="scope.row.attachment" type="primary" :href="scope.row.attachment"
                >下载附件</el-link
              >
              <div v-if="scope.row.attachment" class="line"></div>
            ...
            <div v-if="scope.row.status == 4" class="fcs">
    ...
    >
      ...
      >
        ...
        <el-form-item label="上传附件" prop="file" style="margin-bottom: 0">
          <KzUpload
            ref="upload"
...
const beforeCloseAdd = (done: Function) => {
  //关闭添加弹窗之前
  if (upLoading.value) {
    ElMessageBox.confirm('正在上传中，关闭弹窗可能会导致上传失败，是否继续关闭？', '温馨提示', {
...
const sureDel = () => {
  ...
  delDemand_api({ id: delId.value }).then((res) => {
    if (res.status === 1) {
      delShow.value = false
      getDemandList()
    }
